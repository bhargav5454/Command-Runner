...
export function activate(context: vscode.ExtensionContext) {
    ...
    const addSequenceCommand = vscode.commands.registerCommand('commandRunner.addSequence', () => {
        SequenceEditorManager.createOrShow(context.extensionUri, sequenceProvider);
    });
        ...
        if (selected) {
            ...
            switch (action) {
                case 'Run':
                    ...
                    break;
                case 'Edit':
                    SequenceEditorManager.createOrShow(context.extensionUri, sequenceProvider, selected.sequence);
                    break;
                case 'Delete':
    ...
    const editSequenceCommand = vscode.commands.registerCommand('commandRunner.editSequence', async (item?: CommandSequenceItem) => {
        ...
        if (sequence) {
            SequenceEditorManager.createOrShow(context.extensionUri, sequenceProvider, sequence);
        }
    });
