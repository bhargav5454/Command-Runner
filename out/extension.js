...
function activate(context) {
    ...
    const runSequenceCommand = vscode.commands.registerCommand("commandRunner.runSequence", async (item) => {
        if (item && "steps" in item.sequence) {
            ...
            const selected = await vscode.window.showQuickPick(sequences.map((s) => ({
                label: s.name,
    ...
    const deleteSequenceCommand = vscode.commands.registerCommand("commandRunner.deleteSequence", async (item) => {
        ...
        if (sequenceName) {
            const confirm = await vscode.window.showWarningMessage(`Delete sequence "${sequenceName}"?`, { modal: true }, "Delete", "Cancel");
            if (confirm === "Delete") {
    ...
    const runFromStepCommand = vscode.commands.registerCommand("commandRunner.runFromStep", async (item) => {
        ...
        const startIndex = seq.steps.findIndex((s) => s === step ||
            (s.command === step.command && s.directory === step.directory));
        if (startIndex < 0) {
            vscode.window.showErrorMessage("Step not found in sequence.");
            return;
        }
        const confirm = await vscode.window.showInformationMessage(`Run "${seq.name}" starting from step ${startIndex + 1}?`, "Run", "Cancel");
        if (confirm === "Run") {
            const subSeq = {
    ...
    const runAllSequencesCommand = vscode.commands.registerCommand("commandRunner.runAllSequences", async () => {
        const sequences = sequenceProvider.getSequences();
        if (sequences.length === 0) {
            vscode.window.showInformationMessage("No sequences to run.");
            return;
        }
        const confirm = await vscode.window.showInformationMessage(`Run all ${sequences.length} sequence${sequences.length !== 1 ? "s" : ""}?`, "Run All", "Cancel");
        if (confirm === "Run All") {
            let successCount = 0;
            let failCount = 0;
            for (const sequence of sequences) {
                ...
                if (success) {
                    ...
                    if (failCount > 0) {
                        const continueRun = await vscode.window.showWarningMessage(`Sequence "${sequence.name}" failed. Continue with remaining sequences?`, "Continue", "Stop");
            ...
            vscode.window.showInformationMessage(`Completed: ${successCount} succeeded, ${failCount} failed`);
        }
    });
    // Duplicate Sequence command
    const duplicateSequenceCommand = vscode.commands.registerCommand("commandRunner.duplicateSequence", async (item) => {
        let sequence;
        if (item && "steps" in item.sequence) {
            sequence = item.sequence;
        }
        ...
        vscode.window.showInformationMessage(`Duplicated sequence as "${newName}"`);
    });
    // Export Sequences command
    const exportSequencesCommand = vscode.commands.registerCommand("commandRunner.exportSequences", async () => {
        const sequences = sequenceProvider.getSequences();
        if (sequences.length === 0) {
        ...
        vscode.window.showInformationMessage(`${sequences.length} sequence${sequences.length !== 1 ? "s" : ""} exported to clipboard as JSON`);
    });
    // Import Sequences command
    const importSequencesCommand = vscode.commands.registerCommand("commandRunner.importSequences", async () => {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Imported ${importedCount} sequence${importedCount !== 1 ? "s" : ""}`);
        }
        catch (error) {
    ...
    const refreshViewCommand = vscode.commands.registerCommand("commandRunner.refreshView", () => {
        sequenceProvider.refresh();
        vscode.window.showInformationMessage("Command Runner view refreshed");
    });
    // Show Output command
    const showOutputCommand = vscode.commands.registerCommand("commandRunner.showOutput", () => {
        // This is handled by the executor, but we register it here for the command palette
    });
    context.subscriptions.push(addSequenceCommand, runSequenceCommand, deleteSequenceCommand, manageSequencesCommand, editSequenceCommand, runFromStepCommand, copyStepCommand, runAllSequencesCommand, duplicateSequenceCommand, exportSequencesCommand, importSequencesCommand, refreshViewCommand, showOutputCommand, treeView, executor);
}
